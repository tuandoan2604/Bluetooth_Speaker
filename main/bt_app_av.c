...
static void bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
        ...
        if (rc->meta_rsp.attr_id == 0x01 && rc->meta_rsp.attr_text[0] != '\0')  {
            len = rc->meta_rsp.attr_length;
            for(int i = 0; i < len;i++) {
                Infor[i] = rc->meta_rsp.attr_text[i];
            }
            Infor[len] = '\n';
        }
        else if (rc->meta_rsp.attr_id == 0x02 && rc->meta_rsp.attr_text[0] != '\0'){
            for(int i = 0; i < rc->meta_rsp.attr_length;i++){
                Infor[len+1+i] = rc->meta_rsp.attr_text[i];
            }
            Infor[len+1+rc->meta_rsp.attr_length] ='\0';
            ESP_LOGI(BT_RC_CT_TAG, "DATA %s",Infor);
            task_ssd1306_display_clear();
            task_ssd1306_display_text(&Infor);
        }
        else if (rc->meta_rsp.attr_id == 0x03){
